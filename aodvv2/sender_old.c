...
/* helper functions */
void old_send_udp(void *buffer, size_t length);
static void old_write_packet(struct rfc5444_writer *wr __attribute__ ((unused)),
        struct rfc5444_writer_target *iface __attribute__((unused)),
        void *buffer, size_t length);
static void print_ipv6_addr(const ipv6_addr_t *ipv6_addr);
static uint16_t old_get_node_id(void);


void old_sender_init(void)
{   
    /* experimental */
 ...
 */
static void
old_write_packet(struct rfc5444_writer *wr __attribute__ ((unused)),
        struct rfc5444_writer_target *iface __attribute__((unused)),
        void *buffer, size_t length)
{
    printf("[aodvv2] %s()\n", __func__);
